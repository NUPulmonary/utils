                   ...
                   cores = 1) #cores to run in parallel
{
   ...
   for(cell in cellTypes)
   {
      ...
      {
            sd = c(sd, sum(grepl(sample, cellSub)))
      }
      ...
      sampleData = rbind(sampleData, sd, stringsAsFactors = FALSE)
      ...
      if(length(keep) > 1)
      {
          ...
          cellSub = colnames(object)[object@meta.data[cellSub, compFactor] %in% keep]
      } else
      {
         warning("Skipped -- not enough samples")
      ...
      bulkSampleData = rbind(bulkSampleData, c(cell, bulkN$N), stringsAsFactors = FALSE)
      colnames(bulkSampleData) = bulkSampleDataCols #necessary on first pass (but harmless afterward)

      joinedMat = matrix(data = NA, nrow = nrow(object), ncol = length(samples))
      rownames(joinedMat) = rownames(object)
      colnames(joinedMat) = samples
      for(sample in samples)
      {
          sampleMat = GetAssayData(object, assay = "RNA", slot = "counts")[
            , grepl(sample, colnames(object))
          ]
